...
export default function useTotalAmountOwedToYou(groupId) {
  const { data: user } = useUser();

  return useQuery({
    queryKey: ["total-amount-owed-to-you", groupId, user?.id],
    queryFn: async () => {
      if (!user?.id || !groupId) return 0; // Return 0 if user is not logged in or no groupId provided

      const supabase = supabaseBrowser();

      // Query to get the total amount owed to the user
